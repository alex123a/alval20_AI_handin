...
def compute_forward(states, observations, transitions, emissions):
    ...
    for state in inclusive_range(1, big_n):
        forward[state, 1] = transitions[0, state] * emissions[state][observations[1]]

    for t in inclusive_range(2, big_t):
        for state in inclusive_range(1, big_n):
            forward[state, t] = sum(forward[s, t - 1] * transitions[s, state] * emissions[state][observations[t]] for s in inclusive_range(1, big_n))
    
    forward[f, big_t] = sum(forward[state, big_t] * transitions[state, f] for state in inclusive_range(1, big_n))
